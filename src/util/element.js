/*
 * @Author: wbt
 * @Date: 2021-04-07 10:22:40
 * @LastEditTime: 2021-04-27 13:56:21
 * @LastEditors: wbt
 * @Description: In User Settings Edit
 * @FilePath: /wbt-plugins/src/util/element.js
 */
...
export const createPanel = (params) => {
  ...
  // )
    ...
    panel.setFrame_display(NSMakeRect(mousePos.x - options.width - params.source.frame().size.width,mousePos.y-options.height, options.width, options.height),true)
...
export default class InitContext{
    constructor(context){
        this.pluginSketch = context.plugin.url().URLByAppendingPathComponent("Contents").URLByAppendingPathComponent("Sketch").URLByAppendingPathComponent("library");
    }
    ...
    getImage(size, name) {
        ...
        var imageURL = this.pluginSketch.URLByAppendingPathComponent("toolbar").URLByAppendingPathComponent(name + '.png');
